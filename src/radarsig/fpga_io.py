...
def process_channel_data(data: np.ndarray, n_pulse: int = 14):
    ...
    return (x[:n_total].reshape((pulse_len, n_pulse)), y[:n_total].reshape((pulse_len, n_pulse)))
...
def load_fpga_ram_binary_to_iq(
    i_path: Path, q_path: Path, offset_dtype: int = 0, n_pulse: int = 14
):
